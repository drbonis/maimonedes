...
- **[shipped] Gradient-guided probe generation (#52).** `maimonedes synthesize-probes --strategy gradient` replaces each GP-proposed seed with a gradient-stepped target: numerical-derivative descent direction on the Stage-2 score for the policy's worst-fragility axis (override via `--gradient-axis`), step size defaulting to 0.1 × the GP kernel's length scale, with a GP-uncertainty cap (`2.0 × σ_train_max`) and a violation-threshold stop (default 0.5). The K-NN exemplar text generator + τ check + LLM-as-validator gate still apply — gradient stepping does not bypass quality gating. The original spec wanted the gradient direction from the Riemannian metric; v1 uses the Stage-2 score gradient instead, which is operationally identical for the boundary-finding intent and removes the metric dependency. New rows record `generation_method='gradient_v1'` (the existing `synthesized_probes.generation_method` column is reused as the strategy field — adding a separate `synthesis_strategy` column would be a duplicate).
